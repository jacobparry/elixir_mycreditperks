...
4. Test that everything works by navigating to the root `[umbrella_app]` and running `mix phx.server`.
...
3. Open the file `[umbrella_app]/apps/[database_app]/mix.exs`
  ...
  - Add the following dependencies found on hex.pm to the dependency block:
    ...
    3. `{:postgrex, "~> 0.14.1"}` or <https://hex.pm/packages/postgrex> (for the latest)
...
3. Open the files `[umbrella_app]/apps/[api_app]/mix.exs` and `[umbrella_app]/apps/[ui_app]/mix.exs`
  ...
  - Add the following dependencies found on hex.pm to the dependency block:
    ...
    <https://hex.pm/packages/absinthe> for latest <https://hex.pm/packages/absinthe_plug> for latest <https://hex.pm/packages/absinthe_phoenix> for latest <https://hex.pm/packages/absinthe_relay> for latest
...
8. From `[umbrella_app]`, run the following command:
...
1. Create a `User` Model
  ...
  - Create a new folder and file `models/user.ex` (if it doesnt exist) at `[umbrella_app]/apps/[database_app]/lib//models/user.ex` ``` defmodule Db.Models.User do use Ecto.Schema
  ...
  field(:username, :string) field(:password, :string) field(:email, :string) field(:age, :integer) timestamps()

  ###### #

  end

  @required_fields [:username, :password, :email] @optional_fields [:age]

  def changeset(user, params \ %{}) do user |> cast(params, @required_fields ++ @optional_fields) |> validate_required(@required_fields) end end ```

# 2.1--ecto-models-card

1. Create a `Card` Model

  - Create a new folder and file `models/card.ex` (if it doesnt exist) at `[umbrella_app]/apps/[database_app]/lib/models/card.ex` ``` defmodule Db.Models.Card do use Ecto.Schema
  ...
  @required_fields [:name] @optional_fields []

  def changeset(card, params \ %{}) do card |> cast(params, @required_fields ++ @optional_fields) |> validate_required(@required_fields) end end end ```

# 2.2--ecto-models-user-card

1. Create a `Card` Model

  - Create a new folder (if it doesnt exist) and file `models/card.ex` at `[umbrella_app]/apps/[database_app]/lib/models/user_card.ex` ``` defmodule Db.Models.UserCard do use Ecto.Schema
  ...
  belongs_to(:user, Db.Models.User) belongs_to(:card, Db.Models.Card) timestamps()

  ###### #

  @required_fields [ :user_id, :card_id ] @optional_fields []
...
1. Create a `Perk` Model
  ...
  - Create a new folder (if it doesnt exist) and file `models/perk.ex` at `[umbrella_app]/apps/[database_app]/lib/models/perk.ex` ``` defmodule Db.Models.Perk do use Ecto.Schema
  ...
  field(:type, :string) field(:description, :string) timestamps()

  belongs_to(:card, Db.Models.Card)

  ###### #

  @required_fields [ :type, :description ] @optional_fields []
...
- Create a folder called `models` located at `[umbrella_app]/apps/[database_app]/test/models`.
- Create a file named `user_test.exs` located at `[umbrella_app]/apps/[database_app]/models/user_test.exs`
...
- Once you have done that, create test files for all the models that we created:`card_test.ex`, `perk_test.ex`, `user_card_test.exs`. They will all follow a similar style of testing.

- Add an api endpoint for our tests to hit for testing Graphql queries
...
- Write a simple test that tests our `health` field in our Absinthe Schema file.
